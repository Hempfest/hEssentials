	...
	public static boolean canUseScoreboard() {
	  ...
	  public static boolean didRecieve(Player player)
	  {
	    return Utils.recieved.containsKey(player.getName()) ? 
	      ((Boolean)Utils.recieved.get(player.getName())).booleanValue() : false;
	  }
	  
		//HUD check
		public static boolean hasScore(Player player)
		{
			return Utils.hud.containsKey(player.getName()) ? 
					((Boolean)Utils.hud.get(player.getName())).booleanValue() : false;
		}
